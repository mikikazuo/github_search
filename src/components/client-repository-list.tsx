...
export function ClientRepositoryList({
    ...
    const [hasMore, setHasMore] = useState(initialTotalCount > initialItems.length);
    ...
    const loadMore = async () => {
        ...
        try {
            ...
            setHasMore(
                data.items.length > 0 &&
                    initialItems.length + repositories.length + data.items.length < data.total_count
            );
        } catch (err) {
            setError(err instanceof Error ? err.message : "読み込み中にエラーが発生しました");
        } finally {
            setLoading(false);
        }
    };
